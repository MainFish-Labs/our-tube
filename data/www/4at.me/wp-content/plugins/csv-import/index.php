...
define('WP_CONST_CSV_IMP_VERSION', '1.2');
...
function action_csvimport_admin_init() {
	if (isset($_REQUEST['page']) && ($_REQUEST['page'] == 'csv-import/index.php' || $_REQUEST['page'] == 'page')) {
		...
		wp_localize_script( 'csv-import-js', 'wp_csv_import_translate', translateReqString());
...
function translateReqString(){
	$HelperObj = new WPcsvImport_includes_helper();
	$csv_importStrings = array(
                'chart_msg' => __('NO LOGS YET NOW.','csv-import'),
                'emptyTemplate' => __('Template name is empty','csv-import'),
                'existTemplate' => __('Template Name already exists','csv-import'),
                'errorMsg' => __('Error: ','csv-import'),
                'mandatoryMsg' => __(' - Mandatory fields. Please map the fields to proceed.','csv-import'),
                'generalMsg' => __(' should be mapped.','csv-import'),
                'validateFile' => __('File must be .zip!','csv-import'),
                'reqfdMsg' => __('Fill all mandatory fields.','csv-import'),
                'importProgress' => __('Your Import Is In Progress...','csv-import'),
                'terminate_Import' => __('Import process has been terminated.','csv-import'),
                'continue_Import' => __(' Import process has been continued.','csv-import'),
                'validateRecordnum' => __('Please enter numeric characters only','csv-import'),
		'secure_key' => $HelperObj->create_nonce_key()
	);
	return $csv_importStrings;
}
...
function uploadfile_handle() {
	check_ajax_referer('csvimp_nonce', 'secure_key');
