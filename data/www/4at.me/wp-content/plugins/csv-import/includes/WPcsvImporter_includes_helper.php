...
class WPcsvImport_includes_helper {
	...
	{
		$this->getKeyVals();
		$this->currentUrl = admin_url() . 'admin.php';
	}
	...
	public function getImportDataConfiguration(){
		$importDataConfig = "<div class='importstatus'id='importallwithps_div'>
			<table><tr><td>
			<label id='importalign'>". esc_html__('Import with post status','csv-import') ."</label><span class='mandatory'> *</span></td><td>
			<div style='float:left;margin-right:10px;'>
			<select name='importallwithps' id='importallwithps' style ='width:150px;'onChange='selectpoststatus();' >
			<option value='0'>". esc_html__('Status as in csv','csv-import')."</option>
                        <option value='1'>".esc_html__('Publish','csv-import')."</option>
                        <option value='2'>".esc_html__('Sticky','csv-import')."</option>
                        <option value='3'>".esc_html__('Private','csv-import')."</option>
                        <option value='6'>".esc_html__('Protected','csv-import')."</option>
                        <option value='4'>".esc_html__('Draft','csv-import')."</option>
                        <option value='5'>".esc_html__('Pending','csv-import')."</option>
			</select></div>
			...
			". esc_html__('Select the status for the post  imported, if not defined within your csv .E.g.publish','csv-import')."
			<img src='". WP_CONST_CSV_IMP_DIR."images/help.png' style='margin-top: 6px;float:right;' />
			</span></a> </div>
			</td></tr><tr><td>
			<div id='globalpassword_label' class='globalpassword' style='display:none;padding-left:5px'><label>". esc_html__('Password','csv-import')."</label><span class='mandatory'> *</span></div></td><td>
			<div id='globalpassword_text' style='display:none;'><input type = 'text' id='globalpassword_txt' style ='width:150px;' name='globalpassword_txt' placeholder=".esc_html__('Password for all post','csv-import')."></div></td></tr></table>
			</div>";
	...
	public static function get_fimg_from_URL($f_img, $fimg_path, $fimg_name, $post_slug_value, $currentLimit = null, $logObj = ""){
		...
		if ($rawdata == false) {
			if ($logObj == '') {
				$this->detailedLog[$currentLimit]['image'] = "<b>" .esc_html__('Image','csv-import')." -</b> " .esc_html__('host not resolved','csv-import');
			} else {
				$logObj->detailedLog[$currentLimit]['image'] = "<b>" . esc_html__('Image','csv-import')." -</b>" .esc_html__('host not resolved','csv-import');
			}
		} else {
			...
			fwrite($fp, $rawdata);
			fclose($fp);
			$logObj->detailedLog[$currentLimit]['image'] = "<b>". esc_html__('Image','csv-import')." -</b>" . $fimg_name;
		}
		curl_close($ch);
		return $fimg_name;
	}
		...
		if ($this->titleDupCheck == 'true' && $this->conDupCheck == 'true') 
		 ...
		 // Date format post
                        ...
                        if ($data_array ['post_date'] == null){
                                $data_array ['post_date'] = date('Y-m-d H:i:s');
                                $this->detailedLog[$currentLimit]['postdate'] = "<b>".esc_html__('Date','csv-import')." - </b>" . $data_array ['post_date'];
                        }else{
		...
		if ($this->postFlag) {
			...
			switch ($data_array ['post_status']) {
				case 1 :
					...
					$this->detailedLog[$currentLimit]['poststatus'] = "<b>". esc_html__('Status','csv-import')." - </b>".esc_html__('publish','csv-import');
					break;
				case 2 :
					if ($_SESSION['SMACK_MAPPING_SETTINGS_VALUES']['selectedImporter'] == 'post')
                                        ...
                                        $data_array['post_status'] = 'publish';
                                        $sticky = true;
                                        $this->detailedLog[$currentLimit]['poststatus'] = "<b>". esc_html__('Status','csv-import')." - </b>".esc_html__('sticky','csv-import');
                                        }
                                        else
                                        {
                                        $data_array['post_status'] = 'publish';
				...
				case 3 :
					...
					$this->detailedLog[$currentLimit]['poststatus'] = "<b>". esc_html__('Status','csv-import')." - </b>".esc_html__('private','csv-import');
					break;
				case 4 :
					$data_array ['post_status'] = 'draft';
					$data_array['post_date_gmt'] = $data_array['post_date'];
					$this->detailedLog[$currentLimit]['poststatus'] = "<b>". esc_html__('Status','csv-import')." - </b>".esc_html__('draft','csv-import');
					break;
				case 5 :
					$data_array ['post_status'] = 'pending';
					$data_array['post_date_gmt'] = $data_array['post_date'];
					$this->detailedLog[$currentLimit]['poststatus'] = "<b>". esc_html__('Status','csv-import')." - </b>".esc_html__('pending','csv-import');
					break;
				default :
					...
					if ($data_array['post_status'] != 'publish' && $data_array['post_status'] != 'private' && $data_array['post_status'] != 'draft' && $data_array['post_status'] != 'pending' && $data_array['post_status'] != 'sticky') {
						...
						if ($stripPSF === 0) {
							...
							if ($stripPSL == '}') {
								...
								if (strlen($postpwd) !=0)
								$this->detailedLog[$currentLimit]['poststatus'] = "<b>".esc_html__('Status','csv-import')." - </b>".esc_html__('protected with password','csv-import');
								else
								$this->detailedLog[$currentLimit]['poststatus'] = "<b>". esc_html__('Status','csv-import')." - </b>".esc_html__('publish','csv-import');
							} else {
								$data_array['post_status'] = 'publish';
			...
			if(isset($data_array ['post_author'])){
				...
				if (empty($postauthor) || !$postauthor[0]->ID) {
					...
					$this->detailedLog[$currentLimit]['assigned_author'] = "<b>" .esc_html__('Author - not found (assigned to','csv-import')." </b>" . $admindet[0]->user_login . ")";
					$this->noPostAuthCount++;
				} else {
					$data_array ['post_author'] = $postauthor [0]->ID;
					$this->detailedLog[$currentLimit]['assigned_author'] = "<b>".esc_html__('Author','csv-import')." - </b>" . $postauthor[0]->user_login;
				}
			}
			else{
				$data_array ['post_author'] = 1;
			...
			if ($data_array) {
				if($this->MultiImages == 'true') { // Inline image import feature by fredrick marks
					...
					if($post_id != 0)
					$this->detailedLog[$currentLimit]['post_id'] = "<b>".esc_html__('Created Post_ID','csv-import')." - </b>" . $post_id . " - success";
					else
					$this->detailedLog[$currentLimit]['post_id'] = "<b>".esc_html__('Created Post_ID','csv-import')." - </b>" . $post_id . " - failed";
				}
			}
			unset($postauthor);
			if ($post_id) {
				...
				if (!empty ($tags)) {
					...
					$this->detailedLog[$currentLimit]['tags'] = "<b>".esc_html__('Tags','csv-import')." - </b>" .substr($this->detailedLog[$currentLimit]['tags'], 0, -1);
				}
				...
				if (!empty ($categories)) {
					...
					$this->detailedLog[$currentLimit]['category'] = "<b>".esc_html__('Category','csv-import')." - </b>" .substr($this->detailedLog[$currentLimit]['category'], 0, -1);
	...
	public static function activate() {
		if (!defined('PDO::ATTR_DRIVER_NAME')) {
			echo esc_html__("Make sure you have enable the PDO extensions in your environment before activate the plugin!",'csv-import');
			die;
		}
		global $wpdb;
		...
		foreach($importedTypes as $importedType){
			$querycheck = $wpdb->get_results($wpdb->prepare("select *from smackcsvimp_pie_log where type = %s",$importedType));
	...
	public function addPieChartEntry($imported_as, $count) {
		//add total counts
		global $wpdb;
		$getTypeID = $wpdb->get_results($wpdb->prepare("select * from smackcsvimp_pie_log where type = %s",$imported_as));
	...
	function smack_csv_import_method() {
		...
		$smack_csv_import_method = '<div class="importfileoption">
			...
			<label><span class="radio-icon"><input type="radio" name="importmethod" id="uploadfilefromcomputer" onclick="choose_import_method(this.id);" checked/></span> <span class="header-text" id="importopt">' . esc_html__('From Computer','csv-import') . '</span> </label> <br>
			...
			<span>' . esc_html__('Browse','csv-import') . '</span>
		...
		$smack_csv_import_method .= '<span>
			...
			<label><span class="radio-icon"><input type="radio" name="importmethod" id="dwnldftpfile"  /></span> <span class="header-text" id="importopt">' . esc_html__('From FTP','csv-import') . '</span> </label> <img src="' . WP_CONTENT_URL . '/plugins/' . WP_CONST_CSV_IMP_SLUG . '/images/pro_icon.gif" title="PRO Feature" /> <br>
			</div>
			<div id="boxmethod3" class="method3">
			<label> <span class="radio-icon"><input type="radio" name="importmethod" id="dwnldextrfile"  /></span> <span class="header-text" id="importopt">' . esc_html__('From URL','csv-import') . '</span></label> <img src="' . WP_CONTENT_URL . '/plugins/' . WP_CONST_CSV_IMP_SLUG . '/images/pro_icon.gif" title="PRO Feature" /> <br>
			</div>
			<div id="boxmethod4" class="method4">
			<label><span class="radio-icon"><input type="radio" name="importmethod" id="useuploadedfile"  /></span> <span class="header-text" id="importopt">' . esc_html__('From Already Uploaded','csv-import') . '</span></label> <img src="' . WP_CONTENT_URL . '/plugins/' . WP_CONST_CSV_IMP_SLUG . '/images/pro_icon.gif" title="PRO Feature" /> <br>
			</div>
			</div>
			...
			<span class="advancemediahandling"> <label id="importalign"> <input type="checkbox" name="advance_media_handling" id="advance_media_handling"   onclick = "filezipopen();" /> '.esc_html__("Advance Media Handling",'csv-import').' </label> </span>
	...
	function create_nonce_key(){
		return wp_create_nonce('csvimp_nonce');
	}
	...
	public function getStatsWithDate() {
		...
		foreach($month as $mkey) {
			foreach($year as $ykey) {
				...
				$j = 0;
				$plot = $wpdb->get_results($wpdb->prepare("select inserted,imported_type from smackcsvimp_line_log where imported_type in (%s,%s,%s) and month = %s and year = %s",'Post','Page','Custom Post',$mkey,$ykey));
	...
	public function piechart()
	{
		...
		foreach($imptype as $imp) {
			$OverviewDetails = $wpdb->get_results($wpdb->prepare("select * from smackcsvimp_pie_log where type = %s and value != %d",$imp,0));
