...
?>
	...
	<tr>
	<td>
	<h3><?php echo esc_html__('CSV Import Options','csv-import'); ?></h3>
	...
	<form action='<?php echo esc_url(add_query_arg(array('page' => WP_CONST_CSV_IMP_SLUG.'/index.php', '__module' => sanitize_text_field($_REQUEST['__module']), 'step' => 'mapping_settings'), $impCE->currentUrl));?>' id='browsefile' enctype="multipart/form-data" method='post' name='browsefile'>
	...
	<!-- The global progress bar -->
         <div class="form-group" style="padding-bottom:20px;">
                                ...
                                <input type='button' name='clearform' id='clearform' value='<?php echo esc_attr__("Clear",'csv-import'); ?>' onclick="Reload();"
                                class='btn btn-warning' style="margin-right:15px" />
                                <input type='submit' name='importfile' title = '<?php echo esc_attr__('Next','csv-import');?>' id='importfile' value='<?php echo $impCE->reduceStringLength(esc_attr__("Next",'csv-import'),'Next'); echo (" >>");?>' disabled
	...
	if(check_upload_dir == 'notfound'){
		...
		function prepareUpload(){
			...
			var seckey = wp_csv_import_translate.secure_key;
                        var curraction = '<?php echo sanitize_text_field($_REQUEST['__module']); ?>';
			...
			frmdata.append('secure_key',seckey);
                                        ...
                                        jQuery.each(fileobj,function(objkey,objval){
                                                        jQuery.each(objval,function(o_key,file){
                                                                ...
                                                                if(check_file == "csv"){
                                                                        ...
                                                                        file.uploadedname = filenamecsv[0] + "-<?php echo sanitize_text_field($_REQUEST['__module']); ?>" + ".csv";
                                                                }
                                                                if(check_file == "txt"){
                                                                        var filenametxt = file.uploadedname.split(".txt");
                                                                        file.uploadedname = filenametxt[0] + "-<?php echo sanitize_text_field($_REQUEST['__module']); ?>" + ".txt";
                                                                }
                                                                document.getElementById('upload_csv_realname').value = file.uploadedname;
								document.getElementById('progressbar').value = '100'; 
                                                                var get_version1 = file.name.split("-<?php echo sanitize_text_field($_REQUEST['__module']); ?>");
	...
	<tr>
	<td>
	<form name='mappingConfig' action="<?php echo esc_url(add_query_arg(array('page' => WP_CONST_CSV_IMP_SLUG.'/index.php', '__module' => sanitize_text_field($_REQUEST['__module']), 'step' => 'importoptions'), $impCE->currentUrl));?>"  method="post" onsubmit="return import_csv();" >
	...
	?>
	<h3><?php echo esc_html__('Map CSV to WP fields/attributes','csv-import'); ?></h3>
	...
	<h2><div class="secondformheader"><?php echo esc_html__('Import Data Configuration','csv-import'); ?></div></h2>
        <?php  
	...
	<?php if(isset($_REQUEST['__module']) && sanitize_text_field($_REQUEST['__module'])=='custompost'){ ?>
		<div class='importstatus'>
			...
			<div style = 'float:left'> <label id="importalign"> <?php echo esc_html__('Select Post Type','csv-import'); ?> </label> <span class="mandatory"> * </span> </div>
			...
			<img src="<?php echo esc_url(WP_CONST_CSV_IMP_DIR.'images/help.png');?>" />
			<span class="tooltipCustompost">
			<img class="callout" src="<?php echo esc_url(WP_CONST_CSV_IMP_DIR.'images/callout.gif');?>" />
			<strong><?php echo esc_html__('Select your custompost type','csv-import'); ?></strong>
			<img src="<?php echo esc_url(WP_CONST_CSV_IMP_DIR.'images/help.png');?>" style="margin-top: 6px;float:right;" />
			</span>
			</a><label id='cust_post_empty' style='display:none'><?php echo esc_html__('CUSTOMPOST LIST IS EMPTY','csv-import'); ?></label>
			</div>
			</div>
			<?php } ?>
			...
			<tr>
                        ...
                        <tr>
                        <td class="left_align columnheader" style ="padding-left:170px;"> <b><?php echo esc_html__('WP FIELDS','csv-import'); ?></b> </td><td class="columnheader" style="padding-left:55px;"> <b><?php echo esc_html__('CSV HEADER','csv-import'); ?></b> </td><td> </td><td></td></tr>
                        <?php
...
?>
		...
		<div>
                ...
                <input type='button' id='clear_mapping' title = '<?php echo esc_attr__('Reset','csv-import'); ?>' class='clear_mapping btn btn-warning' name='clear_mapping' value='<?php echo esc_attr__('Reset','csv-import'); ?>' onclick='clearMapping();' style = 'float:left'/>
                </div>
                <div class="mappingactions" >
                <input type='submit' id='goto_importer_setting' title = '<?php echo esc_attr__('Next','csv-import'); ?>' class='goto_importer_setting btn btn-info' name='goto_importer_setting' value='<?php echo $impCE->reduceStringLength(esc_attr__('Next','csv-import'),'Next'); ?> >>' />
                </div>
                </div> 
		</div>
		</div>
		...
		<tr>
		<td>
		<h3><?php echo esc_html__('Settings and Performance','csv-import'); ?></h3>
		...
		<div class="inside">
                <label id="importalign"><input type ='radio' id='importNow' name='importMode' value='' onclick='choose_import_mode(this.id);' checked/> <?php echo esc_html__("Import right away",'csv-import'); ?> </label> 
                                        <label id="importalign"><input type ='radio' id='scheduleNow' name='importMode' value='' onclick='choose_import_mode(this.id);' disabled/> <?php echo esc_html__("Schedule now",'csv-import'); ?> </label>
 ...
 <div id='importrightaway' style='display:block'>
		...
		<ul id="settings">
		<li>
		<label id="importalign"><input name='duplicatecontent' id='duplicatecontent' type="checkbox" value=""> <?php echo esc_html__('Detect duplicate post content','csv-import'); ?></label> <br>
		<input type='hidden' name='wpnoncekey' id='wpnoncekey' value='<?php echo $nonce_Key; ?>' />
		<label id="importalign"><input name='duplicatetitle' id='duplicatetitle' type="checkbox" value="" > <?php echo  esc_html__('Detect duplicate post title','csv-import'); ?></label> <br>
		 <label id="importalign"><?php echo esc_html__('No. of posts/rows per server request','csv-import'); ?></label> <span class="mandatory" style="margin-left:-13px;margin-right:10px">*</span> <input name="importlimit" id="importlimit" type="text" value="1" placeholder="10" onblur="check_allnumeric(this.value);"></label> <?php echo $impCE->helpnotes(); ?><br>
			<span class='msg' id='server_request_warning' style="display:none;color:red;margin-left:-10px;"><?php echo esc_html__('You can set upto','csv-import'); ?> <?php echo $_SESSION['SMACK_MAPPING_SETTINGS_VALUES']['totRecords']; ?><?php echo esc_html__('per request.','csv-import'); ?></span>
		...
		<input type="hidden" id="terminateaction" name="terminateaction" value="continue" />
                ...
                <label id='importalign'> <input type ='checkbox' id='multiimage' name='multiimage' value = '' > <?php echo esc_html__('Insert Inline Images','csv-import'); ?> </label><br>
                <input type='hidden' id='inlineimagevalue' name='inlineimagevalue' value='none' />
		</li>
		</ul>
		<input id="startbutton" class="btn btn-primary" type="button" value="<?php echo esc_attr__('Import Now','csv-import'); ?>" style="color: #ffffff;background:#2E9AFE;" onclick="importRecordsbySettings();" >
		<input id="terminatenow" class="btn btn-danger btn-sm" type="button" value="<?php echo esc_attr__('Terminate Now','csv-import'); ?>" style="display:none;" onclick="terminateProcess();" />
		<input class="btn btn-warning" type="button" value="<?php echo esc_attr__('Reload','csv-import'); ?>" id="importagain" style="display:none" onclick="import_again();" />
                <input id="continuebutton" class="btn btn-lg btn-success" type="button" value="<?php echo esc_attr__('Continue','csv-import'); ?>" style="display:none;color: #ffffff;" onclick="continueprocess();">
		<div id="ajaxloader" style="display:none"><img src="<?php echo esc_url(WP_CONST_CSV_IMP_DIR.'images/ajax-loader.gif');?>"> <?php echo esc_html__('Processing...','csv-import'); ?></div>
		<div class="clear"></div>
		</form>
                </div>
		<div class="clear"></div>
		...
		<!-- Code Ends Here-->
                   ...
                   <div style="width:100%;">
                                               ...
                                               <tr>
                                               <td>
                                               <h3><?php echo esc_html__("Summary",'csv-import'); ?></h3>
                                                <div id='reportLog' class='postbox'  style='display:none;'>
                                                <input type='hidden' name = 'csv_version' id = 'csv_version' value = "<?php if(isset($_POST['uploaded_csv_name'])) { echo sanitize_text_field($_POST['uploaded_csv_name']); } ?>">
